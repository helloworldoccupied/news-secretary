...
数据管线（20+数据源，BTC/ETH聚焦）：
  ...
  11. 大V观点：9个KOL/媒体RSS（Arthur Hayes、吴说、Willy Woo、10xResearch、Glassnode、Odaily、PANews、The Block、BitcoinMagazine），7天窗口

分析：Claude Sonnet via Anthropic API（矿场视角+核心矛盾+因果链+历史类比+大V观点交叉验证）
...
SOURCE_TIER = {
    ...
    # KOL大V — 个人分析师和华语头部媒体
    'Arthur Hayes': 'T1', 'Wu Blockchain': 'T1', 'Willy Woo': 'T1',
    'The Block': 'T1', 'Glassnode': 'T1', '10xResearch': 'T1', 'BitcoinMagazine': 'T1',
    'Odaily': 'T2', 'PANews': 'T2',
    # CryptoCompare子源默认T3
}
...
def collect_kol_opinions():
    """Step 11: 大V观点采集 — 7个KOL的Substack/Medium/RSS
    1. Arthur Hayes (BitMEX创始人，深度宏观+加密分析)
    2. 吴说区块链 Wu Blockchain (华语加密第一媒体人)
    3. Willy Woo (BTC链上分析专家)
    4. PlanB (S2F模型创始人)
    5. 刘教链 (北航教授，BTC长期价值投资)
    6. Odaily星球日报 (华语头部加密媒体深度文章)
    7. PANews (华语头部加密媒体)
    """
    print('\n[11/11] 大V观点采集（7源）...')
    kol_articles = []
    cutoff = datetime.now(timezone.utc) - timedelta(days=7)  # 7天窗口（个人KOL发文频率低，1周1-2篇）

    KOL_FEEDS = [
        # 个人大V — 深度分析型（发文频率低但含金量极高）
        ('Arthur Hayes', 'https://cryptohayes.substack.com/feed', 'substack', '前BitMEX CEO，宏观+加密深度分析'),
        ('Wu Blockchain', 'https://wublock.substack.com/feed', 'substack', '华语加密第一媒体人，行业信息枢纽'),
        ('Willy Woo', 'https://willywoo.substack.com/feed', 'substack', 'BTC链上数据分析专家'),
        ('10xResearch', 'https://10xresearch.substack.com/feed', 'substack', '量化研究机构，日更级别的数据驱动分析'),
        # 链上数据研究机构（替代已不活跃的PlanB/刘教链）
        ('Glassnode', 'https://insights.glassnode.com/rss/', 'rss', '链上数据分析权威，矿工/持有者行为洞察'),
        # 华语头部加密媒体 — 深度报道+快讯
        ('Odaily', 'https://rss.odaily.news/rss/post', 'rss', '星球日报，华语头部加密媒体深度文章'),
        ('PANews', 'https://www.panewslab.com/zh/rss/newsflash.xml', 'rss', '华语头部加密媒体，项目与行业分析'),
        # 英文头部加密媒体深度文章
        ('The Block', 'https://www.theblock.co/rss.xml', 'rss', '加密行业顶级财经媒体，数据驱动型报道'),
        ('BitcoinMagazine', 'https://bitcoinmagazine.com/feed', 'rss', 'BTC专注媒体，挖矿/技术/政策深度'),
    ]

    for kol_name, feed_url, feed_type, _desc in KOL_FEEDS:
        try:
            req = Request(feed_url, headers=UA)
            raw = urlopen(req, timeout=15).read()
            root = ET.fromstring(raw)
            ...
            for item in items:
                ...
                desc = (item.findtext('description') or item.findtext('content:encoded', namespaces={'content': 'http://purl.org/rss/1.0/modules/content/'}) or item.findtext('atom:content', namespaces=ns) or item.findtext('atom:summary', namespaces=ns) or '').strip()
                pub = item.findtext('pubDate') or item.findtext('atom:published', namespaces=ns) or item.findtext('atom:updated', namespaces=ns) or ''
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                # 清理HTML标签，保留更多文本给LLM分析
                desc_clean = re.sub(r'<[^>]+>', '', desc)[:1500]
                kol_articles.append({
                    'kol': kol_name,
                    'title': title,
                    'summary': desc_clean,
                ...
                count += 1
                if count >= 5:  # 每个KOL最多5篇
                    break
            print(f'  [{kol_name}] {count}篇')
        except Exception as e:
            print(f'  [{kol_name}] 采集失败: {e}')

    # 按时间倒序
    kol_articles.sort(key=lambda x: x.get('ts') or 0, reverse=True)
    print(f'  [汇总] 共{len(kol_articles)}篇大V文章')
    return kol_articles
...
## ⚠️ 最高优先级：可行性研究报告级别的分析深度
你的报告的每一个章节都必须达到**可行性研究报告（Feasibility Study）**的分析深度，而不是新闻摘要或市场简评。
具体标准：
- 每个分析点必须有**定量数据支撑**（不是"上升"而是"从X上升至Y，涨幅Z%"）
- 每个判断必须有**因果传导链**（事件→机制→一阶效应→二阶效应→对矿场的影响）
- 每个结论必须有**历史类比验证**（给出具体日期、具体价格、当时的后续走势）
- 每个风险和机会必须有**量化影响评估**（如果X发生，对矿场收入影响约$Y/天）
- 每个策略建议必须有**触发条件和执行方案**（不是"注意风险"而是"如果BTC跌破$XX,XXX，建议在Y天内出售Z%的挖矿产出"）
- 段落不要惜字——一个分析点如果值得写就值得写透，300-500字/段是正常的
- 宁可一个点分析得深入透彻，也不要十个点浅尝辄止

## 董事长背景
- 经营大型BTC ASIC矿场（F2Pool 26+子账户，多地机房）
- 同时持有ETH仓位
- 关心：BTC价格对挖矿利润的影响、算力/难度对产出的影响、矿工经营周期、能源成本盈亏平衡
- 阅读设备：笔记本电脑或iPad，不限篇幅，深度分析优先于篇幅控制
...
## 篇幅要求（最高优先级，违反=报废重写）
- **总报告不少于12000字**。少于12000字的报告不合格，少于10000字直接报废
- 各章节内部字数标准（不要在输出的标题中标注字数，这是内部指标）：
  - 快照200字、核心矛盾800字、矿工经济学1500字
  - **BTC深度分析1000字**（链上+衍生品+期权三个维度各300+字，不要偷工减料）
  - **ETH分析500字**（价格+衍生品+ETH/BTC相对强弱+配置建议）
  - **宏观环境800字**（DXY+美债+黄金+美股四个维度各200字）
  - 新闻每条250字、大V观点1000字、DeFi 300字、风险矩阵800字
- ⚠️ **后半截章节（BTC深度、ETH、宏观）是最容易偷工减料的地方**。你必须对这三个章节投入与前半截同等的分析深度。如果你发现自己在这些章节只写了1-2段话就想收笔，停下来，加入更多历史类比、更多因果推演、更多量化估算
- 宁可写得过于详细也不要写得过于简略——董事长是在iPad上阅读，有的是时间
- **输出的章节标题必须干净**：只写"## 第X部分：标题名"，不要在标题中附加"（XXX字）"等标注

## ⛔ 绝对禁止（违反任何一条=报废）
- **禁止省略任何章节**。10个部分（市场快照→核心矛盾→矿工经济学→BTC深度→ETH→宏观→新闻→大V→DeFi→风险矩阵）必须全部输出，一个都不能少
- **禁止写"以下部分略"、"根据需求展开"、"篇幅有限不再展开"** 等任何形式的截断/省略语句。报告必须从第一部分完整写到第十部分
- **禁止合并章节**。每个部分必须有独立的 ## 标题
- **禁止用"详见上文"、"参考前述分析"** 等偷懒引用。每个章节独立完整，即使有些数据在前面出现过，也要在新的语境下重新分析
- 你有足够的output token（24000 tokens），不需要省略任何内容

## 输出格式要求（笔记本电脑/iPad大屏幕阅读）
- 用 ## 大标题（不要用###，大V观点章节内可用### 区分不同KOL）
- 关键数字全部**加粗**
- 不限段落长度和篇幅，深度分析优先于篇幅控制
- 段落间用空行分隔
...
要求（最高优先级——每个章节都必须达到可行性研究报告的深度）：
1. 先给出紧凑一行式市场快照（最关键数据一目了然）
2. 找到今日1-2个核心矛盾，围绕矿场经营视角展开深入分析
3. **可行性研究报告级别的分析深度**——每个数据点都要：① 量化描述（具体数字+变化幅度） ② 因果传导链（3-4步推演） ③ 历史类比验证（具体日期+价格+后续走势） ④ 对矿场经营的影响量化（影响每TH/s收入$X.XX或月收入$XXK）
4. 新闻逐条深度分析（200+字因果链+历史先例+影响量化），每条新闻是一篇微型研究报告
5. 策略建议围绕矿场经营（持币/卖出/扩算力/电费），每个建议必须有触发条件+执行方案+回撤预案
6. 段落充分展开——一个论点如果值得分析就值得分析透彻，不要缩写不要省略

## 报告结构

**第一部分：市场快照**（紧凑一行式，放最前面，一屏看完关键数据）
格式如下（每项一行，数据紧凑）：
- ⚡ **BTC变现信号**: 🔴立即卖出 / 🟡逢高减仓 / 🟢持币待涨 / ⚪观望（基于Puell+周期+链上+宏观综合判断，附一句话理由）
...
- 今日1-2个核心矛盾的完整论证：
  - 矛盾的本质是什么？（用一句话定义）
  - 支持多方的数据证据（至少3个数据点，每个量化）
  - 支持空方的数据证据（至少3个数据点，每个量化）
  - 历史上类似矛盾是如何解决的？（具体日期+价格+后续走势）
  - 你的方向判断及逻辑链（5步以上的因果推演）
- 对矿场经营的具体建议（每条必须有触发条件+执行方案+回撤预案）：
  - **BTC变现时机判断**（⚠️核心决策）：
    - 信号灯判断依据：综合Puell Multiple周期位置 + 链上矿工抛售行为 + 宏观流动性环境 + 技术面支撑/阻力
    - 具体卖出策略：在什么价格区间卖出？分几批卖？每批卖多少比例？间隔多长时间？
    - 如果当前是🟢持币：什么条件变化时应切换到🟡减仓？（具体价格/指标阈值）
    - 如果当前是🟡减仓：建议卖出比例（每日/每周产出的XX%）、通过哪种方式（OTC/交易所/场外）
    - 如果当前是🔴卖出：紧急程度、目标在几日内完成、是否需要对冲
  - 是否适合扩算力/购买新矿机？回本周期估算（当前BTC价格+难度+电费下）
  - 电费策略建议（满负荷/降负荷/关机）：具体到当BTC跌破$XX,XXX时切换策略
  - ETH持仓策略：持有/减仓/增仓的具体条件和比例

**第三部分：矿工经济学**
- **算力竞争格局**：全网算力当前值 vs 7日均值 vs 40天趋势方向 → 推演对小/中/大矿工的差异化影响 → 大矿工（如董事长26+子账户规模）在这个竞争格局下的优势/劣势
- **单位产出量化**：当前每TH/s每日BTC产出精确计算 → 与30天前对比 → 趋势外推（如果算力继续按当前速度增长，1/3/6个月后每TH/s产出预估）
- **难度调整深度分析**：下次调整预估幅度 → 对矿工收入的精确影响（调整前后每日收入差额$XXX） → 历史上类似幅度的难度调整后矿工行为（关机/扩容/抛售）
- **矿工收入解构**：日收入=区块奖励+交易费 → 交易费占比趋势 → Mempool拥堵对交易费的边际贡献 → 当前交易费年化收益率
- **Puell Multiple深度解读**：当前值 → 历史分位数（过去3年中处于什么位置） → 与价格周期的关系 → 上一次相近值时的日期和后续走势 → 对当前矿场的操作含义
- **矿工行为链上证据**：矿工地址余额变化 → 是否在加速转出到交易所 → 与历史底部/顶部矿工行为的对比
- **盈亏平衡详细模型**：
  - S19 XP（21.5 J/TH）在电费0.35元/度下的关机价
  - S21（15 J/TH）在电费0.35元/度下的关机价
  - 如果BTC跌至关机价，对全网算力和难度的连锁反应预估
- **对矿场的具体经营建议**：基于以上全部分析，给出3-5条可执行的建议，每条附带"如果X则Y"的条件触发逻辑

**第四部分：BTC深度分析**
- **链上数据**：NVT Ratio当前值+历史分位数+与价格周期关系 → 活跃地址趋势+与价格背离信号+历史上类似背离的后续走势（具体日期和价格）
- **衍生品**：三所资金费率对比（找分歧、分歧的含义、历史上类似分歧的后续） → 全市场费率分布+正费率占比趋势 → OI变化+价格方向组合解读（4种组合场景分析） → Taker买卖比趋势方向
- **期权**：Put/Call变化+历史类比 → DVOL vs 历史波动率的价差含义 → Max Pain引力+到期日对短期价格的影响

**第五部分：ETH分析**
- ETH价格/涨跌幅+ETH/BTC汇率走势
- ETH衍生品（费率、期权情绪）和与BTC衍生品的对比
- ETH/BTC相对强弱的周期含义（历史上ETH/BTC走强/走弱分别预示什么）
- ETH作为董事长第二大持仓的配置建议（增/减/持的条件）

**第六部分：宏观环境**
- **DXY深度分析**：当前值+趋势方向 → DXY与BTC的历史相关系数 → 近期DXY走强/走弱的驱动因素 → 对BTC价格的量化压制/利好（DXY每变动1%对BTC的历史影响幅度）
- **美债收益率**：10Y当前值+趋势 → 实际利率（名义-通胀预期）对风险资产的压制机制 → 收益率曲线形态（倒挂/正常化）的经济周期含义
- **黄金 vs BTC**：相关系数+近期走势对比 → "数字黄金"叙事在本轮周期的验证/证伪 → 黄金创新高但BTC未跟随（或跟随）的含义
- **美股联动**：标普500+纳斯达克与BTC的近期相关性 → 科技股领涨/下跌是否传导到加密 → "脱钩"还是"再耦合"的判断

**第七部分：行业动态深度分析**
- 从新闻数据中选取最重要的10-15条（RED级全部入选，YELLOW级择优）
- 每条新闻必须按以下5步结构深度分析（总字数不少于250字，重要事件500字+）：

格式：
[T1/T2/T3] 来源名 | 🔴/🟡/🟢 重要性级别
**标题**

① **事件概述**：2-3句话说清楚发生了什么，包含关键数字（涉及金额/数量/百分比都要写）
② **历史先例**：引用1-2个具体的历史类似事件。每个必须包含：具体日期、具体价格/数据、当时的后续走势（至少追踪到3个月后的结果）。例如："类似2024年1月SEC批准BTC现货ETF（BTC从$46,000涨至$73,000，+58%，ETF首月流入$40B），矿工收入从$2500万/天跃升至$6000万/天"
③ **因果传导链**：事件 → 传导机制 → 一阶效应 → 二阶效应 → 三阶效应。每一步都要有逻辑解释，不能跳步。例如："CleanSpark出售97%产出 → 矿工需覆盖$0.045/kWh电费 → 短期BTC供给增加约300BTC/月 → 场外OTC市场承压 → 但这是成本驱动的理性行为非恐慌性抛售 → 大型矿企都在这么做说明行业进入精细化运营阶段"
④ **对BTC价格的影响判断**：方向（利好/利空/中性）+ 时间窗口（短期1-7天/中期1-3月/长期3-12月）+ 影响量级估算（对价格影响±$XXX或±X%）
⑤ **对矿场经营的影响**：对算力、难度、电费成本、挖矿收益的具体量化影响。例如："如果该政策落地，预计6个月内全网算力下降5-10%，难度相应调整，每TH/s日收入增加约$0.002"

禁止写少于200字的简评。重要事件（RED级）写500字以上。每条新闻都是一篇微型可行性分析报告。
- 分3类整理：①监管与政策 ②项目与技术进展 ③市场事件

**第八部分：大V观点与市场共识**

⚠️ 极其重要：你必须逐个分析数据中每一个有文章的KOL/媒体。数据中有Arthur Hayes、Wu Blockchain、Willy Woo、10xResearch、Glassnode、Odaily、PANews、The Block、BitcoinMagazine等来源的文章。每个有文章的来源都必须单独分析，不能合并，不能跳过。每个KOL的分析不少于200字：

格式（对每个KOL重复此结构）：
### [KOL名字] — [一句话定位，如"前BitMEX CEO，宏观+加密深度分析"]

**核心论点提炼**：用2-3句话概括该KOL近期文章的核心判断（看多/看空/具体逻辑），不要照抄标题
**数据交叉验证**：将该KOL的观点与你前面分析的链上数据、衍生品数据、宏观数据做对比。哪些数据支持他的观点？哪些数据反驳他的观点？给出具体数据点
**历史准确度评估**：如果你了解该KOL的历史预测记录，简要评价其可信度（如"Arthur Hayes 2024年初看多BTC到$100K，实际ATH $73K，方向正确但幅度偏高"）
**与你的结论异同**：该KOL的观点与你在第二部分的核心判断是一致还是矛盾？如果矛盾，谁更可能正确？为什么？
**对矿场的参考价值**：这个观点对矿场经营决策（持币/卖出/扩算力/电费）有什么具体参考意义？

最后：
### 市场共识度总结
- 看多阵营 vs 看空阵营 vs 中立观望：各有谁？人数比？
- 共识度评估：高度一致（>80%同向）/ 适度分歧（50-80%）/ 严重分裂（<50%）
- 当大V高度一致看多时反而要警惕（逆向指标），严重分裂时说明市场处于关键节点
- 综合大V观点后，是否修正你之前的核心判断？如果修正，修正幅度和理由
...
**第十部分：风险矩阵**
- **Top 3风险**（每个风险不少于150字）：
  - 风险描述 + 触发条件（具体到价格/指标阈值）
  - 对矿场的量化影响（影响每日收入$XXX / 影响月利润XX%）
  - 发生概率评估（基于当前数据，不是主观猜测）
  - 对冲/应对方案（具体到操作步骤）
- **Top 3机会**（每个机会不少于150字）：
  - 机会描述 + 入场条件（具体到价格/指标阈值）
  - 预期收益量化
  - 实现概率评估
  - 执行方案
- **反向观点**（不少于300字，必写）：提出至少一个与你主结论相反的情景：
  - 完整的反向论证链（不少于5步因果推演）
  - 支持反向观点的数据证据（至少2个数据点）
  - 在什么条件下你的主判断会被证伪（具体阈值）
  - 如果被证伪，矿场应该采取的应急方案
...
def call_llm_analysis(data_context):
    ...
    采用分段生成策略：将10个章节分成2批调用，确保每批都能获得充分的深度。
    单次调用时LLM倾向于压缩后半段内容，分批调用可解决此问题。
    """
    from llm_engine import call_llm

    # ====== 第一批：章节1-5（市场快照→ETH分析）======
    print('\n[批次1/2] 调用LLM：章节1-5（市场快照→ETH分析）...')
    batch1_prompt = CRYPTO_ANALYST_USER.replace('{data_context}', data_context)
    # 在prompt末尾追加批次指令
    batch1_instruction = """

⚠️ 重要：本次只输出前5个部分（第一部分到第五部分），不要输出第六到第十部分。
你必须输出：
1. ## 第一部分：市场快照（200字+）
2. ## 第二部分：核心矛盾与判断（800字+）
3. ## 第三部分：矿工经济学（1500字+）
4. ## 第四部分：BTC深度分析（1000字+，链上+衍生品+期权三个维度各300+字）
5. ## 第五部分：ETH分析（500字+）

在第五部分结束后停止。不要写第六到第十部分。每个部分都必须达到上面标注的最低字数。"""
    batch1_msg = batch1_prompt + batch1_instruction

    batch1 = call_llm(
        system_prompt=CRYPTO_ANALYST_SYSTEM,
        user_prompt=batch1_msg,
        model='sonnet',
        fallback='deepseek',
        max_tokens=16000,
        timeout=360,
    ...
    print(f'  批次1完成: {len(batch1)} 字符')

    # ====== 第二批：章节6-10（宏观→风险矩阵）======
    print('\n[批次2/2] 调用LLM：章节6-10（宏观→风险矩阵）...')
    batch2_instruction = f"""你是专门服务BTC矿场董事长的首席分析师。你正在撰写加密投研日报的后半部分。

前5个部分已经完成（不要重复），你现在只需要输出第六到第十部分。

## 前面已完成的分析结论摘要（供你参考保持一致性）：
{batch1[:2000]}
...
⚠️ 重要：本次只输出后5个部分，不要重复前面的内容。
你必须输出：
6. ## 第六部分：宏观环境（800字+，DXY+美债+黄金+美股四个维度各200字）
7. ## 第七部分：行业动态深度分析（每条新闻250字+深度因果分析）
8. ## 第八部分：大V观点与市场共识（每个KOL分析200字+）
9. ## 第九部分：DeFi与稳定币（300字+）
10. ## 第十部分：风险矩阵（800字+，含Top3风险+Top3机会+反向观点300字）

每个部分都必须达到上面标注的最低字数。不要偷工减料。"""

    batch2 = call_llm(
        system_prompt=CRYPTO_ANALYST_SYSTEM,
        user_prompt=batch2_instruction,
        model='sonnet',
        fallback='deepseek',
        max_tokens=16000,
        timeout=360,
    ...
    if not batch2:
        print('  ❌ 批次2失败，使用仅批次1的结果')
        return batch1

    print(f'  批次2完成: {len(batch2)} 字符')

    # ====== 合并两批结果 ======
    combined = batch1.rstrip() + '\n\n---\n\n' + batch2.lstrip()
    print(f'\n合并后总长度: {len(combined)} 字符')

    # ====== 质量检查 ======
    combined = _enforce_section_quality(combined, data_context)

    return combined


def _enforce_section_quality(analysis, data_context):
    """硬性质量检查：检查各章节字数，不达标的调用LLM逐章补写"""
    import re
    from llm_engine import call_llm
    ...
    section_pattern = re.compile(
        r'(##\s*第[一二三四五六七八九十]+部分[：:].+?)(?=##\s*第[一二三四五六七八九十]+部分[：:]|---\s*$|\Z)',
        re.DOTALL
    )
    sections = section_pattern.findall(analysis)

    if not sections:
        print('  ⚠️ 质量检查：无法解析章节结构，跳过')
        return analysis

    print(f'\n=== 质量检查（{len(sections)}个章节）===')
    short_sections = []
    for section_text in sections:
        for key, min_len in SECTION_MINS.items():
            if key in section_text:
                section_len = len(section_text)
                status = '✅' if section_len >= min_len else '❌'
                print(f'  {status} {key}: {section_len}字 (最低{min_len})')
                if section_len < min_len:
                    # 提取标题
                    title_match = re.search(r'##\s*(第[一二三四五六七八九十]+部分[：:].+?)(?:\n|$)', section_text)
                    title = title_match.group(1).strip() if title_match else key
                    short_sections.append((key, title, section_text, min_len))
                break
    ...
    # 逐章补写不达标的部分
    print(f'\n⚠️ {len(short_sections)}个章节不达标，启动逐章补写...')

    for key, title, original_text, min_len in short_sections:
        print(f'\n  补写 {title} (当前{len(original_text)}字 → 目标{min_len}字)...')

        expand_prompt = f"""你之前写的报告中，"{title}"章节严重不足（只有{len(original_text)}字，最低要求{min_len}字）。

请只重写这一个章节。要求：
- 必须输出{min_len}字以上，目标{int(min_len * 1.5)}字
- 以"## {title}"开头
- 加入大量历史类比（具体日期+具体数字+当时的后续走势）
- 加入因果链推演（每一步都有逻辑解释）
- 加入量化影响估算（对矿场收入/利润的具体美元影响）
- 不要写其他章节的内容
- 深度分析，不要惜字

原始不合格内容（仅供参考，不要简单扩写，要重写）：
{original_text}
...
{data_context[:8000]}"""
        ...
        expanded = call_llm(
            system_prompt="你是专门服务BTC矿场董事长的首席分析师。现在补写报告中严重不足的章节。写得越深入越好，不要惜字。",
        ...
        if expanded and len(expanded) > len(original_text):
            analysis = analysis.replace(original_text, expanded)
            print(f'    ✅ 补写完成: {len(expanded)}字')
        else:
            print(f'    ⚠️ 补写失败或未改善，保留原文')

    print(f'\n质量检查后总长度: {len(analysis)} 字符')
    return analysis
...
def format_data_context(market, onchain, mining, funding, oi_ls, options, defi, macro, sentiment, news, kol_opinions=None):
    """将所有数据格式化为Claude的输入"""
    sections = []
    ...
    # 大V观点（KOL Opinions）— 给LLM充分的素材做深度分析
    s = '## 大V观点（近7天KOL最新文章，按KOL分组）\n'
    s += 'KOL背景：Arthur Hayes=前BitMEX CEO宏观+加密深度分析 | Wu Blockchain=华语加密第一媒体人 | Willy Woo=BTC链上数据分析专家 | 10xResearch=量化研究机构数据驱动 | Glassnode=链上数据分析权威 | Odaily=华语头部媒体 | PANews=华语头部媒体 | The Block=英文顶级财经媒体 | BitcoinMagazine=BTC专注媒体(挖矿/技术/政策)\n\n'
    if kol_opinions:
        # 按KOL分组展示，每个KOL的文章放在一起
        from collections import OrderedDict
        kol_groups = OrderedDict()
        for art in kol_opinions:
            kol = art.get('kol', '未知')
            if kol not in kol_groups:
                kol_groups[kol] = []
            kol_groups[kol].append(art)

        for kol_name, articles in kol_groups.items():
            tier = SOURCE_TIER.get(kol_name, 'T2')
            s += f'### [{tier}] {kol_name}（{len(articles)}篇）\n'
            for idx, art in enumerate(articles, 1):
                title = art.get('title', '')
                summary = art.get('summary', '')[:1200]  # 给LLM更多内容
                ts = art.get('ts')
                time_str = ''
                if ts:
                    dt = datetime.fromtimestamp(ts, BJT)
                    time_str = dt.strftime('%m-%d %H:%M BJT')
                s += f'{idx}. **{title}** ({time_str})\n'
                if summary:
                    s += f'   {summary}\n'
            s += '\n'
        s += f'共{len(kol_opinions)}篇文章，覆盖{len(kol_groups)}个KOL/媒体\n'
    else:
        s += '暂无近7天的大V更新\n'
    sections.append(s)
...
def main():
    print(f'=== 加密货币投研日报 v2.4 (Two-Batch-Generation + Quality-Gate) ===')
    ...
    # Step 11: 大V观点采集
    kol_opinions = collect_kol_opinions()
    ...
    data_context = format_data_context(
        market, onchain, mining, funding, oi_ls, options, defi, macro, sentiment, news, kol_opinions
    )
