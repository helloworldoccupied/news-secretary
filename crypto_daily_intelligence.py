...
数据管线（20+数据源）：
  ...
  10. 新闻快讯：6源聚合（BlockBeats快讯+深度/CryptoCompare/CoinDesk/CoinTelegraph/OKX公告）
...
**第九部分：行业动态与项目资讯**
- 从6大新闻源（BlockBeats/CryptoCompare/CoinDesk/CoinTelegraph/OKX公告）提取最重要的10-15条
- 分3类整理：①监管与政策 ②项目进展（上线/升级/合作/融资） ③市场事件（黑客/清算/大额转账）
- 每条新闻给出影响判断（🟢利好/🔴利空/🟡中性 + 影响量级：高/中/低）
- 特别关注：新币上线、协议升级、大额融资、监管动向——这些直接影响交易决策
...
def format_data_context(market, onchain, mining, funding, oi_ls, options, defi, macro, sentiment, news):
    ...
    if news:
        ...
        # 中文源优先，然后英文源
        cn_sources = [k for k in by_source if any(c in k for c in ['BlockBeats', 'OKX'])]
        en_sources = [k for k in by_source if k not in cn_sources]
        idx = 1
        for src in cn_sources + en_sources:
            items = by_source[src]
            s += f'\n### [{src}]（{len(items)}条）\n'
            for n in items[:12]:
                s += f"{idx}. {n['title']}\n"
