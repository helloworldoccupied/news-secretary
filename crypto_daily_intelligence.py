...
数据管线（20+数据源，BTC/ETH聚焦）：
  ...
  11. 大V观点：7个KOL的Substack/Medium/RSS（Arthur Hayes、吴说、Willy Woo、PlanB、刘教链、Odaily、PANews）
...
def collect_kol_opinions():
    ...
    print('\n[11/11] 大V观点采集（7源）...')
    kol_articles = []
    cutoff = datetime.now(timezone.utc) - timedelta(hours=72)  # 72小时内的文章

    KOL_FEEDS = [
        ('Arthur Hayes', 'https://cryptohayes.substack.com/feed', 'substack'),
        ('Wu Blockchain', 'https://wublock.substack.com/feed', 'substack'),
        ('Willy Woo', 'https://willywoo.substack.com/feed', 'substack'),
        ('PlanB', 'https://medium.com/feed/@100trillionUSD', 'medium'),
        ('刘教链', 'https://blockcoach.substack.com/feed', 'substack'),
        ('Odaily', 'https://rss.odaily.news/rss/post', 'rss'),
        ('PANews', 'https://www.panewslab.com/zh/rss/newsflash.xml', 'rss'),
    ]

    for kol_name, feed_url, feed_type in KOL_FEEDS:
        try:
            req = Request(feed_url, headers=UA)
            raw = urlopen(req, timeout=12).read()
            root = ET.fromstring(raw)
...
**第八部分：大V观点与市场共识**（⚠️ 新增重要章节）
- 基于大V观点数据中的KOL最新文章，提炼核心观点
- 逐人分析（Arthur Hayes、吴说、Willy Woo、PlanB、刘教链等有文章的KOL）：
  - 该KOL的核心论点是什么？
  - 与你的数据分析是否一致？如果有分歧，谁更可能正确？为什么？
  - 该观点对矿场经营有什么参考价值？
- 最后给出"市场共识度"判断：多数大V看多/看空/分歧？这对短期走势意味着什么？
- 如果没有KOL数据，跳过本章节
...
def format_data_context(market, onchain, mining, funding, oi_ls, options, defi, macro, sentiment, news, kol_opinions=None):
    ...
    s = '## 大V观点（72小时内KOL最新文章）\n'
    if kol_opinions:
        for idx, art in enumerate(kol_opinions[:15], 1):
            kol = art.get('kol', '未知')
            tier = SOURCE_TIER.get(kol, 'T2')
            title = art.get('title', '')
            ...
            s += f"{idx}. [{tier}] {kol} ({time_str})\n"
            s += f"   标题: {title}\n"
            if summary:
                s += f"   摘要: {summary}\n"
        s += f'\n共{len(kol_opinions)}篇KOL文章\n'
    else:
        s += '暂无72小时内的大V更新\n'
    sections.append(s)
