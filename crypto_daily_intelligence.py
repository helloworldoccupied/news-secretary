...
**第七部分：行业动态**（⚠️ 逐条深度分析，不是简单列表）
- 从新闻数据中选取最重要的10-15条
- 每条新闻按以下格式深度分析：
  [T1/T2/T3来源可信度] 来源名 | 🔴/🟡/🟢 重要性
  标题
  80-150字因果链分析：这条新闻发生了什么 → 为什么重要 → 传导机制 → 对BTC价格/矿场经营/ETH持仓的具体影响
...
def split_and_push(analysis_text, date_str):
    ...
    header = (
        ...
        f'> 恐贪指数仪表盘 · BTC/ETH涨跌图 · 40天算力+每TH收益走势 — 图表嵌在分析文字中\n\n'
        f'---\n\n'
    )
...
def main():
    print(f'=== 加密货币投研日报 v2.0 (Mining-Focused) ===')
