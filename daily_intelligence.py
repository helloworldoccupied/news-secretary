#!/usr/bin/env python3
"""
每日市场情报系统 v3.1 — Daily Market Intelligence
首席分析师（Chief Analyst）· Claude Sonnet 深度分析
每日08:00 BJT 自动运行（GitHub Actions cron: '0 0 * * *' UTC）

v3.1 更新（2026-03-01）：
- 新增BlockBeats(律动)快讯：链上大额转账、项目动态、行业快讯（类似OKX项目资讯）
- 步骤从9步增加到10步
...
## 关键事件
（从新闻、OKX公告、BlockBeats快讯中提取3-5个真正重要的事件，每个事件用一段话说清：发生了什么→为什么重要→对投资的影响。链上大额转账（鲸鱼动向）、OKX新币上线/下架、重大项目进展如有交易机会要提。与投资无关的忽略。）
...
def build_final_report(analysis, crypto, macro, ashare, defi, news, calendar, hr_chart=None):
    ...
    header = f'# 每日市场情报\n**{now.strftime("%Y-%m-%d")} · 首席分析师简报 v3.1**\n'
    ...
    footer = (f'\n---\n'
              f'*{health.get("ok",0)}源 · {total}条新闻(过滤后) · Claude Sonnet · v3.1*\n'
...
def main():
    now = datetime.now(BJT)
    print(f'\n{"="*60}')
    print(f'  每日市场情报 v3.1 — 首席分析师(Claude Sonnet)')
