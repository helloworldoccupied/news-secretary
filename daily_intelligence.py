...
# ============================================================
def build_data_context(crypto, macro, ashare, defi, news, calendar, okx_events=None):
    now = datetime.now(BJT)
...
## 关键事件
（从新闻和OKX公告中提取3-5个真正重要的事件，每个事件用一段话说清：发生了什么→为什么重要→对投资的影响。OKX新币上线/下架/网络升级等公告如果有交易机会也要提。与投资无关的新闻不要提。）
...
def build_final_report(analysis, crypto, macro, ashare, defi, news, calendar, hr_chart=None):
    ...
    footer = (f'\n---\n'
              ...
              f'*数据: OKX(行情+公告)/Binance/CoinGecko/DefiLlama/mempool/Yahoo/东方财富*')
...
def main():
    ...
    print('\n[1/9] 加密货币(OKX+Binance+CoinGecko)...')
    ...
    print('\n[9/9] Claude Sonnet 统一深度分析...')
    data_ctx = build_data_context(crypto, macro, ashare, defi, news, calendar, okx_events=okx_events)
